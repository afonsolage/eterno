        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de, T: ChunkStorageType> serde::de::Visitor<'de> for ArrayVisitor<T> {
            ...
            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                formatter.write_fmt(format_args!("an array of length {}", BUFFER_SIZE))
            }
                ...
                A: serde::de::SeqAccess<'de>,
            {
                ...
                if !vec.is_empty() && vec.len() != BUFFER_SIZE {
...
impl<T: ChunkStorageType> Serialize for ChunkStorage<T> {
    ...
    where
        S: serde::Serializer,
    {
        let mut seq = serializer.serialize_seq(Some(BUFFER_SIZE))?;
